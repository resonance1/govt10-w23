...
When you see a white background you are writing text.  You can use Markdown here.  However we will also be interested in writing code to explore and summarize data.  Code is placed in a *chunk.*  Chunks start and end with three grave accents (the same key as the tilde): ```  
`chunks` are interactive and allow to you run code. When you execute code within the notebook, the results appear beneath the code. To run a chunk, press the *Run* button (play button) within the chunk or by placing your cursor inside it and pressing *Cmd+Shift+Enter*. 
...
R also contains _functions_. For instance, the function sqrt() calculates a square root:
```{r}
```
...
1) Create two new code chunks (using the green 'Insert' button in the top right, or by pressing *Cmd+Option+I*).

2) In the first chunk find the sum of 75, 83, 93, 34, 75 and 81 using math operators (+). Then try to find the sum using the function _sum()_. You can look up how to use the function _sum()_ by typing ?sum in the console (bottom of your screen). 
